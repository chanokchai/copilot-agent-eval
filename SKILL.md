...
- `scripts/build_tracker.py` — generates the complete 4-sheet Excel tracker (`pip install openpyxl`, then `python scripts/build_tracker.py [output.xlsx]`). This file is the shared, generic **engine**, committed to a shared GitHub skill repo — it ships with demo data only (3 illustrative agents) and must **never** contain anyone's real agent data. Real agents belong in `scripts/local_agents.py` (same folder, gitignored, project-local, never committed) defining `extra_rows` / `extra_items` / optionally `extra_checklists` in the same shapes as the engine's demo data — `build_tracker.py` merges them in automatically at build time if the file exists (see the confirmation gate in Quick mode step 4). The script prints a pass-rate self-check after saving; confirm those numbers match the story you put in the data.
...
4. **Ask for explicit confirmation before persisting an agent — default is throwaway.** This choice only controls whether the agent is saved to `scripts/local_agents.py` for next time; it never controls whether anything reaches git (real agent data is never committed, period — see below). Ask: "Is this a real agent evaluation you want to keep tracking? (y/N)". Treat anything other than an explicit "y"/"yes" as **No** — a blank reply, silence, "n", or any wording that isn't an unambiguous yes (including contradictory phrasing like the word "dummy"/"test" appearing anywhere in the same request) all resolve to No. Never infer "yes" from context or from other parts of the same message; ask again if genuinely unclear rather than guessing.
   - **No / default (throwaway):** build the workbook from a one-off list that is never written into `scripts/local_agents.py` — generate the report to a plain output folder and treat both as disposable.
   - **Explicit Yes (real):** append the agent's `extra_rows` / `extra_items` / checklist entry to `scripts/local_agents.py` (create it from `build_tracker.py`'s demo-data shapes if it doesn't exist yet), then rebuild. This file is gitignored — appending to it is a local save, not a commit.
   - The committed engine (`build_tracker.py`) changes **only** for genuine mechanism work (new capability, layout/bug fix, documentation) — never to record a specific agent's data. When you do touch it for a real engine change, say so explicitly and treat the commit/push as its own decision, separate from this per-agent question.
...
- Default every new agent to **throwaway** (not persisted anywhere) and require an explicit "y"/"yes" confirmation before appending it to `scripts/local_agents.py`. Blank/unclear answers, and contradictory wording (e.g. "dummy" alongside "real"), both mean No.

**Never:**
- Put real agent data (`rows`/`items`/checklist entries) into the committed `build_tracker.py` — it always goes into the gitignored `scripts/local_agents.py` instead, whether the agent is real or throwaway. `build_tracker.py` only changes for genuine engine/mechanism work.
